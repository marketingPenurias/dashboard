...
export async function loader({ request, context }: Route.LoaderArgs) {
	...
	if (!tenantId) {
		return { needsTenantSelection: true as const, retentionRows: [] as RetentionRow[], economy: null as EconomyRow | null };
	}

	const [retentionRows, economyRows] = await Promise.all([
		analyticsRpc<RetentionRow>(context, "ng_get_cohort_retention", { p_tenant_id: tenantId }),
		analyticsRpc<EconomyRow>(context, "ng_get_token_economy", { p_tenant_id: tenantId }),
	]);
